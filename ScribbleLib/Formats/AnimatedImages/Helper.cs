...
internal class Helper
{
	...
	internal static int ConvertEndian(int i) =>
		BitConverter.ToInt32(ConvertEndian(BitConverter.GetBytes(i)), 0);

	/// <summary>
	///     Convert big-endian to little-endian or reserve
	/// </summary>
	internal static uint ConvertEndian(uint i) =>
		BitConverter.ToUInt32(ConvertEndian(BitConverter.GetBytes(i)), 0);

	/// <summary>
	///     Convert big-endian to little-endian or reserve
	/// </summary>
	internal static short ConvertEndian(short i) =>
		BitConverter.ToInt16(ConvertEndian(BitConverter.GetBytes(i)), 0);

	/// <summary>
	///     Convert big-endian to little-endian or reserve
	/// </summary>
	internal static ushort ConvertEndian(ushort i) =>
		BitConverter.ToUInt16(ConvertEndian(BitConverter.GetBytes(i)), 0);
